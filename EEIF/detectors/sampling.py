...
class VSSampling(Sampling):
    ...
    def __init__(
            self,
            num_samples,
            lower_bound=simple_Size,
            upper_bound=simple_Size,
            vs_type='FIX',
            bootstrap=False,
            bagging=None):
    ...
    def draw_samples(self, data):
        ...
        for i in range(self._num_samples):
            ...
            if self._bagging is not None:
                transformed_data = self._bagging_instances[i].get_transformed_data(
                    data)
            sampled_data = []
